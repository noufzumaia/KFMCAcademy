...
{
    public partial class trResetPass : System.Web.UI.Page
    {
		...
		protected void Page_Load(object sender, EventArgs e)
		{
			...
			{
				GUIDvalue = Request.QueryString["traineeId"];
				if (GUIDvalue != null)
				{
					SqlCommand cmd = new SqlCommand("select * from traineeForgotPass where guidId='" + GUIDvalue + "'", con);
					con.Open();
					SqlDataAdapter sda = new SqlDataAdapter(cmd);
					sda.Fill(dt);
					if (dt.Rows.Count != 0)
					{
						traineeId = Convert.ToInt32(dt.Rows[0][2]);
					}
					else
					{
		...
		protected void btRecPass_Click(object sender, EventArgs e)
		{
			if (txtNewPass.Text != "" && txtConfirmPass.Text != "" && txtNewPass.Text == txtConfirmPass.Text)
			{
				using (SqlConnection con = new SqlConnection(CS))
				{
					SqlCommand cmd = new SqlCommand("update trainee set traineePassword='" + txtNewPass.Text + "' where traineeId='" + traineeId + "'", con);
					con.Open();
					cmd.ExecuteNonQuery();
					SqlCommand cmd2 = new SqlCommand("delete from traineeForgotPass where traineeId='" + traineeId + "'", con);
